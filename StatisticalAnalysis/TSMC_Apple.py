...
apple_direction = np.sign(np.diff(apple_interp))

# Cross-correlation for direction analysis
lags = np.arange(-len(tsmc_direction) + 1, len(tsmc_direction))
cross_corr = correlate(tsmc_direction, apple_direction, mode='full')
...
if best_lag > 0:
    # Apple lags TSMC: shift Apple forward
    apple_shifted = np.pad(apple_direction, (best_lag, 0), mode='constant', constant_values=0)[:len(tsmc_direction)]
    tsmc_shifted = tsmc_direction
elif best_lag < 0:
    # Apple leads TSMC: shift TSMC forward
    tsmc_shifted = np.pad(tsmc_direction, (-best_lag, 0), mode='constant', constant_values=0)[:len(apple_direction)]
    apple_shifted = apple_direction
else:
...
valid_indices = ~np.isnan(tsmc_shifted) & ~np.isnan(apple_shifted)
direction_correlation, direction_p_value = pearsonr(tsmc_shifted[valid_indices], apple_shifted[valid_indices])

# Apply the lag to the original YoY growth data
if best_lag > 0:  # Apple lags TSMC
    lagged_tsmc = tsmc_interp[:-best_lag]
    lagged_apple = apple_interp[best_lag:]
elif best_lag < 0:  # Apple leads TSMC
    lagged_tsmc = tsmc_interp[-best_lag:]
    lagged_apple = apple_interp[:best_lag]
else:  # No lag
    lagged_tsmc = tsmc_interp
    lagged_apple = apple_interp

# Calculate Pearson correlation for lagged data
lagged_correlation, lagged_p_value = pearsonr(lagged_tsmc, lagged_apple)
...
plt.plot(common_time[1:], apple_shifted, label="Apple Direction of Change (Shifted)", color="green", linestyle="--")

# Labels and legend
plt.title(f"Direction of YoY Growth Changes (TSMC vs. Apple, Lag = {best_lag} Quarters)")
...
plt.plot(range(len(lagged_apple)), lagged_apple, label="Lagged Apple YoY Growth", color="green", linestyle="--")

# Labels and legend
plt.title(f"Lagged TSMC and Apple YoY Growth (Lag = {best_lag} Quarters)")
...
# Plotting YoY Growth of TSMC and Apple
plt.figure(figsize=(14, 8))
...
plt.scatter(apple_x, apple_y, color="green", label="Apple Actual Data Points", alpha=0.5)

# Add labels and legend
plt.title("YoY Growth of TSMC and Apple")
plt.xlabel("Time (Years)")
plt.ylabel("YoY Growth (%)")
plt.legend()
plt.grid(True)
